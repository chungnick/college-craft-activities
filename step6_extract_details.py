...
parser = argparse.ArgumentParser(description="Generate detailed summary using Gemini.")
...
Given the RAW TEXT below, produce exactly ONE paragraph that is informational and easy to skim.

Requirements:
- Capture all important, activity-relevant details (who/what/logistics) if present. Omit details regarding location and date/time.
- Do not invent facts; if something is unknown, omit it rather than guessing.
- Keep it as a single cohesive paragraph, with a minimum of 3 sentences (no bullets, no headings).

Format the output as a valid JSON object containing a single key "enhanced_description".
"""
...
def generate_summary(content: str, client: genai.Client) -> Dict:
...
def main():
    ...
    if not existing_results:
        print("No results.json found. Run previous steps first.")
        sys.exit(1)
    ...
    for r in existing_results:
        if not args.repeat and "enhanced_description" in r:
             continue
    ...
    try:
        ...
        iterator = tqdm(rows_to_process, desc="Extracting Details", unit="row")
    except ImportError:
    ...
    for row in iterator:
        row_id = row["id"]
        # print(f"Generating summary for {row_id}...")
        
        content = get_combined_md_content(row_id)
        if not content:
            continue

        summary_data = generate_summary(content, client)
        
        updated_results[row_id].update(summary_data)
        processed_count += 1
    ...
    print(f"Step 5 complete. Processed {processed_count} rows.")
